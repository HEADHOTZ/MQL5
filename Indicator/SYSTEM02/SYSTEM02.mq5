   ...
   for(int i = 0;i < uncalculatedBar;i++)
     {
      ...
      CopyOpen(Symbol(),PERIOD_CURRENT,0,DAY+10,close);

      double spreadPoint = spread[0] *_Point;

      double diffBuy = (close[DAY+1] - sarArray[DAY+1]) / _Point;
      double diffSell = (sarArray[DAY+1] - close[DAY+1]) / _Point;
      double tpBuy = (close[DAY+1] - sarArray[DAY+1]) * TP;
      double tpSell = (sarArray[DAY+1] - close[DAY+1]) * TP;

      double risk = MONEY * (RISK / 100);

      if(Symbol() == "XAUUSD")
        {
         lotBuy = NormalizeDouble(risk / ((close[DAY+1] - sarArray[DAY+1]) * 100),2);
         lotSell = NormalizeDouble(risk / ((sarArray[DAY+1] - close[DAY+1]) * 100),2);
        }
      else
         if(Symbol() == "XAGUSD")
           {
            lotBuy = NormalizeDouble(risk / ((close[DAY+1] - sarArray[DAY+1]) * 5000),2);
            lotSell = NormalizeDouble(risk / ((sarArray[DAY+1] - close[DAY+1]) * 5000),2);
           }
         else
           {
      ...
      //---BUY CONDITION   ---//
      if(close[DAY+1] > sarArray[DAY+1])
        {
         ENBuffer[i] = open[DAY];
         SLBuffer[i] = sarArray[DAY+1] - spreadPoint;
         TPBuffer[i] = ((ENBuffer[i] - sarArray[DAY+1]) * TP) + ENBuffer[i];
      ...
      else if(close[DAY+1] < sarArray[DAY+1])
        {
         ENBuffer[i] = open[DAY];
         SLBuffer[i] = sarArray[DAY+1] + spreadPoint;
         TPBuffer[i] = ENBuffer[i] - ((sarArray[DAY+1] - ENBuffer[i])*TP);
